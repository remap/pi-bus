...
from iot_trust.iot_identity_manager import IotIdentityManager
...
def readConfig():
        configFile = os.path.dirname(__file__)+'config2.cfg'
...
class NDServer(Thread):
	...
	self.keychain.setFace(self.face)
        ...
        if logger is not None:
            ...
            logFormat = "%(levelname)-10s %(asctime)-10s %(funcName)-20s\n\t%(message)s"
            self.logger = logging.getLogger("NDNBusPublisher")
