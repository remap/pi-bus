...
def index(req):
	...
	#logDatabase(preXML, locXML, vehicleID, passengerCount, eta, predictable, lastTime, status)
def logDatabase(predictionXML, locationXML, nextBusRoute, nextBusOccupancy, nextBusETA, nextBusPredictable, status, sampleTime):
#def logDatabase(predictionXML, locationXML, nextBusRoute, nextBusOccupancy, nextBusETA, nextBusPredictable, sampleTime, preTime):

    #connect to mongoDB
    #connection = Connection()
    #db = connection.test
    #collection = db.test
    data = post={"time":_time.time(), "predictions":predictionXML, "vehicleLocations":locationXML, "nextBusRoute":nextBusRoute,"nextBusOccupancy":nextBusOccupancy,"nextBusETA":nextBusETA, "nextBusPredictable":nextBusPredictable, "status":status, "sampleTime":sampleTime}
    ...
    #, "sampleTime": sampleTime, "predictedTime":preTime }
